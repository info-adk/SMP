...
def load_validationset():
    validation_user = []
    filename = "/home/iiip/Desktop/SMP/user_id/task2_10.txt"   #user_id-------
    lines = open(filename).readlines()
    for i in range(0,len(lines), 1):
        line = lines[i].strip()
        validation_user.append(line)
...
def blog_tag(blog_id, keyword_dict):
    blog_dir = '/home/iiip/Desktop/SMP/blog/blog_10/'        #blog_unfenci----------
    keyword_blog_score = {}
...
def blog_act(user_id):
    blog_act_dir = '/home/iiip/Desktop/SMP/user_blog/task2_10/'      #user_behavious-----------
...
def doc2vec_sim(blog_id):
    test_text = open('/home/iiip/blog_fenci/blog_fenci_10/' + blog_id + '.txt').read().split(" ") #blog_fenci---------
...
## 读取验证集用户
validation_user = load_validationset()
file_dir = '/home/iiip/Desktop/SMP/blog_title/task2_10/'   #user_title--------
flag2 = 0
for one in range(0, len(validation_user), 1):


        # validation_user[one] ='U0012131'
        user_file = file_dir + validation_user[one] + ".txt"
        ## 初始化不同权重得分
        # print user_file
        total_score = {}
        ...
        for title in titles:
            ...
            ## 检查每一篇博客关键词的出现情况
            blog_tag_list, blog_keyword_list=blog_tag(blog_id, keyword_dict)
