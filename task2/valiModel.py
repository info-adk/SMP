...
def load_validationset():
    validation_user = []
    filename = "data/ValidationSet.txt"
    lines = open(filename).readlines()
    for i in range(0,len(lines), 1):
        line = lines[i].strip()
...
def blog_act(user_id):
    blog_act_dir = 'data/testuser_blog/'
...
def doc2vec_sim(blog_id):
    test_text = open('data/testblogfenci/' + blog_id + '.txt').read().split(" ")
